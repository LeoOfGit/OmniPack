...
class PipPanel(BasePanel):
    ...
    def _build_pip_ui(self):
        ...
        self.setup_widget = RuntimeSetupWidget("python")
    ...
    def start_scan(self):
        ...
        try:
            ...
            if not envs:
                self._log("No Python environments found. Please add one or install a new one.", "error")
                self.setup_widget.show()
    ...
    def _update_runtime_in_env(self, env_path: str):
        ...
        if env_type == "venv" and os.name == "nt":
            msg = (
                ...
                f"This will first update the system {cycle_display} installation via winget if necessary,\n"
                f"then upgrade this virtual environment.\n\n"
    ...
    def _on_installer_done(self, env_path: str, success: bool, message: str):
        ...
        if success:
            self._log("Runtime installer completed. Refreshing...", "system")
        elif message:
