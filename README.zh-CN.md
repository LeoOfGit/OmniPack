...
### ⛑️ 安全更新智能：约束感知与构建变体识别
OmniPack 能智能评估每个更新的安全性。
- **约束感知自动选中**：开启"Outdated"过滤时，若某个包的最新版本违反了其依赖者的版本约束（如 `mpmath 1.4.1` 破坏了 `sympy` 要求的 `<1.4`），该包**不会被自动选中**，同时显示橙色 `⚠` 图标解释原因。
- **构建变体识别**：自动识别 PEP 440 本地版本后缀（`+cu132`, `+cpu`, `+rocm5.6` 等）。若更新将改变包的硬件构建类型（如 CUDA 切到 CPU），会显示蓝色 `🔀` 图标提醒。
- **二次确认保障**：点击标记风险的更新按钮会弹出详细的风险对话框，确认后方可继续执行。
