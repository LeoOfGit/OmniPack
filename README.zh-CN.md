...
### ✏️ 环境重命名与设置编辑 (v14 全新引入)
无需进入冗长的配置对话框去修改环境配置：
- **右键上下文菜单**：右键点击 Pip/Npm 环境卡片标题，可以直接选择 **“✏️ 重命名环境”** 或 **“⚙️ 编辑设置”**。
- **自动定位高亮**：点击“编辑设置”后，主程序会自动唤起统一设置对话框，并自动高亮且滚动到该环境对应的行，大幅减少操作路径。

### 🚀 Winget 自身一键升级 (v14 全新引入)
支持对 Winget 包管理器自身（App Installer）的审计与一键升级：
- **版本直观呈现**：卡片直接显示 Winget 当前的具体版本号（如 `Winget v1.9.25200`）。
- **一键执行升级**：若检测到 Winget 有可用更新，卡片头部会显示闪烁的 **`Wg`** 按钮并显示升级路径。点击即可弹窗确认，通过内置终端以一键式的命令行逻辑自动升级。

### 📋 全局可复制对话框文本与包详情导出 (v14 全新引入)
- **对话框可复制**：全局注入事件过滤器，使所有弹出的 QMessageBox 和 QDialog 提示文本均支持鼠标悬停、选择与复制，方便复制异常报错与路径。
- **包配置一键复制**：包配置对话框增加了 **“Copy Details”** 按钮，支持一键将该包的元数据（包名、ID、源、版本、路径）拷贝到系统剪贴板。
