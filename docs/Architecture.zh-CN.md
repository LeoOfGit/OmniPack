...
- `core/dep_resolver.py` - **依赖拓扑解析引擎**。通过子进程运行 `importlib.metadata` 脚本，构建完整依赖图（requires/required_by），并合并到 `Package` 对象中；同时自动创建缺失依赖的”幽灵”包条目。针对 redundant (如同时存在 system 与 user 目录) 同名包进行特异性依赖树合并重构与就近匹配路由，保障其各自完备的拓扑树不发生丢失。
- `core/network_proxy.py` - **代理路由与注入层**。提供 `urlopen()` 自定义 opener（按目标域名启用代理）、`merge_env_for_command()` 将代理环境变量注入子进程、以及代理连通性测试。
- `core/npm_spec.py` - **NPM 规范解析器**。解析 `@scope/name@tag` 格式 of 包规范字符串，提取包名与 dist-tag。
...
2. **环境管理 (Environment Management)**：
    - **快速入口与多通道导入**: 在 Pip 面板与 Npm 面板的环境列表底部新增了常驻的“➕ Add Environment”快捷按钮。点击会触发添加菜单，支持选择“项目/虚拟环境文件夹”、“python.exe/package.json 配置文件”、“手动输入路径”以及新引入的 **“Batch Paste... (批量粘贴)”**。批量粘贴允许用户直接将从 Explorer 或 Everything 中复制的多行路径一次性导入，大幅提高操作效率。
